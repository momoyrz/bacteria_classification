...
class CrossAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        # q是0branch的class token产生，k，v是1branch的patch产生，做注意力操作，用来融合。
...
class CrossAttentionBlock(nn.Module):
    ...
    def forward(self, x):
        x = x[:, 0:1, ...] + self.drop_path(self.attn(self.norm1(x)))   # x[:, 0:1, ...] class token
        if self.has_mlp:
...
class MultiScaleBlock(nn.Module):
    ...
    def forward(self, x):
        outs_b = [block(x_) for x_, block in zip(x, self.blocks)]  # x -> list:2 [2, 401, 96],[2, 197, 192]
        # outs_b -> list:2 [2, 401, 96],[2, 197, 192] block就是Transformre层
        proj_cls_token = [proj(x[:, 0:1]) for x, proj in zip(outs_b, self.projs)]    # x[:, 0:1]把class token拿出来
        # proj_cls_token -> list:2 [2, 1, 192], [2, 1, 96]将class token维度变换一下
        outs = []
        for i in range(self.num_branches):
            tmp = torch.cat((proj_cls_token[i], outs_b[(i + 1) % self.num_branches][:, 1:, ...]), dim=1)  # 将0branch的class token和1barch的patch token拼接在一起
            tmp = self.fusion[i](tmp)    # Cross Attention操作
            reverted_proj_cls_token = self.revert_projs[i](tmp[:, 0:1, ...])           # 重新将class token的dim变回来
            tmp = torch.cat((reverted_proj_cls_token, outs_b[i][:, 1:, ...]), dim=1)   # 将变回来的class token重新和自己拼接
            outs.append(tmp)
        return outs              # out -> list: 2 [2, 401, 96],[2, 197, 192]
...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=(224, 224), patch_size=(8, 16), in_chans=3, num_classes=1000, embed_dim=(192, 384),
        ...
        if not isinstance(img_size, list):
            img_size = to_2tuple(img_size)
        self.img_size = img_size           # img_size: [240, 224]

        num_patches = _compute_num_patches(img_size, patch_size)   # num_patches = (img_size / patch_size)^2
        ...
        if hybrid_backbone is None:
            self.pos_embed = nn.ParameterList([nn.Parameter(torch.zeros(1, 1 + num_patches[i], embed_dim[i]))
                                               for i in range(self.num_branches)])    # 构建位置编码
    ...
    def forward_features(self, x):   # x -> [2, 3, 240, 240] [B, C, H, W]
        B, C, H, W = x.shape         # B=2, C=3, H=W=240
        xs = []
        for i in range(self.num_branches):    # self.num_branches = 2
            x_ = torch.nn.functional.interpolate(x, size=(self.img_size[i], self.img_size[i]), mode='bicubic') \
                if H != self.img_size[i] else x        # 上采样或下采样  self.img_size = [240, 224]
            tmp = self.patch_embed[i](x_)              # tmp -> [B, patch_szie, embed_dim]
            cls_tokens = self.cls_token[i].expand(B, -1, -1)  # 构造cls_tokens -> [B, 1, embed_dim]
            tmp = torch.cat((cls_tokens, tmp), dim=1)         # 将cls_tokens和token拼接 tmp -> [B, 1 + patch_size, embed_dim]
            tmp = tmp + self.pos_embed[i]                     # 加上位置编码
            tmp = self.pos_drop(tmp)                          # drop out操作 tmp -> [B, 1, 1 + patch_size, embed_dim]
            xs.append(tmp)        # xs -> list:2

        for blk in self.blocks:      # 进入blocks层
            xs = blk(xs)             # 进入blk模块

        # NOTE: was before branch token section, move to here to assure all branch token are before layer norm
        xs = [self.norm[i](x) for i, x in enumerate(xs)]    # xs -> list: 2 [2, 401, 96] [2, 197, 192]
        out = [x[:, 0] for x in xs]                         # 提取class token out -> list:2 [2, 96] [2 192]

        return out

    def forward(self, x):              # x -> [2, 3, 240, 240]
        xs = self.forward_features(x)  # 提取特征，这里的特征指的就是class token. xs -> [2, 96] [2, 192]
        ce_logits = [self.head[i](x) for i, x in enumerate(xs)]  # 全连接层，用来分类 ce_logits -> [2, 1000] [2, 1000]
        ce_logits = torch.mean(torch.stack(ce_logits, dim=0), dim=0)     # 计算平均概率
        return ce_logits

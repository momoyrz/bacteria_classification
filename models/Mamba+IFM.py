...
class InteractionFusionMamba(nn.Module):
    """
    交互融合Mamba模块（Interaction Fusion Mamba, IFM）
    实现论文中多模态特征交互的核心机制
    """

    def __init__(self, modal1_dim, modal2_dim, output_dim):
        """
        初始化多模态交互模块

        Args:
        - modal1_dim: 第一个模态的特征维度
        - modal2_dim: 第二个模态的特征维度
        - output_dim: 融合后的特征维度
        """
        super().__init__()

        # 特征投影层：对齐不同模态的特征维度
        self.modal1_proj = nn.Linear(modal1_dim, output_dim)
        self.modal2_proj = nn.Linear(modal2_dim, output_dim)
    ...
    def forward(self, modal1_features, modal2_features):
        ...
        Args:
        - modal1_features: 第一个模态的特征
        - modal2_features: 第二个模态的特征

        Returns:
        - 融合后的特征
        """
        # 特征投影：将不同模态的特征映射到相同维度
        proj_modal1 = self.modal1_proj(modal1_features)
        proj_modal2 = self.modal2_proj(modal2_features)

        # 准备门控输入：拼接特征
        gate1_input = torch.cat([proj_modal1, proj_modal2], dim=-1)
        gate2_input = torch.cat([proj_modal2, proj_modal1], dim=-1)
        ...
        # 特征调制：使用门控权重抑制冗余特征
        weighted_modal1 = proj_modal1 * gate1_weights
        weighted_modal2 = proj_modal2 * gate2_weights

        # 特征拼接
        combined_features = torch.cat([weighted_modal1, weighted_modal2], dim=-1)
...
class MambaVision3D(nn.Module):
    ...
    def forward_features(self, x):
        ...
        for i, level in enumerate(self.levels):
            ...
            if i > 0:
                # 创建交互融合模块
                ifm = InteractionFusionMamba(
                    modal1_dim=fine_grained_features[-1].shape[1] // 2,  # 上一层特征的一半
                    modal2_dim=xo.shape[1] // 2,  # 当前层特征的一半
                    output_dim=xo.shape[1]  # 输出维度与当前层相同
                )

                # 将特征均分为两个模态
                last_modal1, last_modal2 = torch.chunk(fine_grained_features[-1], 2, dim=1)
                curr_modal1, curr_modal2 = torch.chunk(xo, 2, dim=1)

                # 特征交互
                xo = ifm(last_modal1, curr_modal1)

            fine_grained_features.append(xo)
            outs.append(xo)
